...
namespace sunny
{
	namespace graphics
	{
		...
		Sprite::Sprite(float x, float y, float width, float height, directx::Texture2D* texture)
		: Renderable2D(maths::vec3(x, y, 0.0f), maths::vec2(width, height), maths::vec4(1, 1, 1, 1))
		{
			m_texture = texture;
		}
