...
namespace sunny
{
	namespace graphics
	{
		...
		Sprite::Sprite(float x, float y, directx::Texture2D* texture, PIVOT pivot)
		: Renderable2D(maths::vec3(x, y, 0.0f), maths::vec2((float)texture->GetWidth(), (float)texture->GetHeight()), maths::vec4(1, 1, 1, 1), pivot)
		{
			m_texture = texture;
		}
