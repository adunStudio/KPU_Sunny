...
namespace sunny
{
	...
	{
		/* STATIC */
		const Shader* Shader::s_currentlyBound[2] = { nullptr, nullptr };
		...
		{
			s_currentlyBound[m_dimension] = this;

			Context::GetDeviceContext(m_dimension)->VSSetShader(m_data.vertexShader, NULL, 0);
			Context::GetDeviceContext(m_dimension)->PSSetShader(m_data.pixelShader, NULL, 0);

			Context::GetDeviceContext(m_dimension)->VSSetConstantBuffers(0, m_VSConstantBuffersCount, m_VSConstantBuffers);
			Context::GetDeviceContext(m_dimension)->PSSetConstantBuffers(0, m_PSConstantBuffersCount, m_PSConstantBuffers);
		}
		...
		void Shader::SetVSSystemUniformBuffer(unsigned char* data, unsigned int size, unsigned int slot)
		{
			...
			Context::GetDeviceContext(m_dimension)->Map(cbuffer, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &msr);
			...
			Context::GetDeviceContext(m_dimension)->Unmap(cbuffer, NULL);
		}
