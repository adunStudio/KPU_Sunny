...
namespace sunny
{
	namespace directx
	{
		...
		void VertexBuffer::SetLayout(const BufferLayout& bufferLayout)
		{
			...
			const Shader* shader = Shader::CurrentlyBound(m_dimension);
		...
		void* VertexBuffer::GetPointerInternal()
		{
			...
			Context::GetDeviceContext(m_dimension)->Map(m_bufferHandle, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &m_mappedSubresource);
		...
		{
			Context::GetDeviceContext(m_dimension)->Unmap(m_bufferHandle, NULL);
		}
		...
		void VertexBuffer::Bind()
		{
			...
			Context::GetDeviceContext(m_dimension)->IASetInputLayout(m_inputLayout);
			Context::GetDeviceContext(m_dimension)->IASetVertexBuffers(0, 1, &m_bufferHandle, &stride, &offset);
